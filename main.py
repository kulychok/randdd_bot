...
print("MongoDB version is %s" %
            random_bot_client.server_info()['version'])

numbers = [0, 0]
digits_pattern = re.compile(r"^[0-9]+ [0-9]+$", re.MULTILINE)

gifs = ['https://t.me/randchoosecont/80',
        'https://t.me/randchoosecont/81']
...
def content_query(query):
    coin_flipping_query(query)
    random_number_query(query)


def random_number_query(query):
    global numbers
    try:
...
def menu(message):
    ...
    # else:
    #     bot.send_message(message.chat.id, random.choice(lg.emojis))
    # if message.text.split()[0] == "V69N72M95S02":
...
def callback_inline(call):
    ...
    emodji_result = []
    for res in result:
        emodji_result.append("{!s}️⃣".format(res))
    if call.message:
        if call.data == "start_rand_nums":
            ...
            bot.edit_message_text(
                ...
                text=lg.result[lg.language].format(numbers[0], numbers[1], ''.join(emodji_result)),
                parse_mode=telegram.ParseMode.MARKDOWN_V2
            )
